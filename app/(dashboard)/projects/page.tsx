            ...
            return (
              ...
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  <span className="w-2.5 h-2.5 rounded-sm shrink-0" style={{ backgroundColor: project.solution?.color ?? '#9CA3AF' }} />
                  <span className="font-medium text-sm text-gray-900 truncate group-hover:text-blue-600 transition-colors">{project.name}</span>
                  {isAtRisk && <AlertTriangle size={13} className="shrink-0 text-red-500" />}
                </div>
                <div className="text-sm text-gray-500 truncate">{project.client?.name ?? <span className="text-gray-300">—</span>}</div>
                <div>
                  {project.solution ? (
                    <span className="text-xs px-2 py-0.5 rounded-full font-medium truncate inline-block max-w-[130px]"
                ...
                <div className="text-center">
                  <span className="text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">{project._count.tasks}</span>
                </div>
                <div className="text-xs text-gray-500">{project.eventDate ? formatDate(project.eventDate) : <span className="text-gray-300">—</span>}</div>
                <div className="text-xs text-gray-500">{project.approvedBudget ? formatCurrency(Number(project.approvedBudget)) : <span className="text-gray-300">—</span>}</div>
              </Link>
            )
          })}
        </div>
