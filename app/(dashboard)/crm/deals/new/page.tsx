...
function NewDealForm({ initialLeadId }: { initialLeadId: string | null }) {
  ...
  async function create() {
    ...
    if (res.ok) {
      ...
      router.push(`/crm/deals/${d.id ?? d.data?.id}`)
    }
    setSaving(false)
  }
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2">
            <label className="block text-xs font-medium text-gray-700 mb-1">Título do Deal *</label>
            <input autoFocus value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="Ex: Projeto Holografia Itaú 2025" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
        ...
        <div className="flex gap-3 pt-2">
          <button onClick={create} disabled={!form.title || saving} className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-sm font-medium px-5 py-2 rounded-lg">
